...
module.exports = {
  ...
  execute(client, { args, guild, member, channel, message }) {
    ...
    if (!toBan) return channel.send("Veuillez mentionner le membre ou mettre son ID")
    else
      toBan =
    ...
    if (!delay) {
        ...
        guild.member(toBan).ban(reason);
        if (delayResolver == false) return channel.send('Veuillez définir un temps correcte (s, m, h, d, w)')
        channel.send(`${toBan} a bien été banni!`)
        setTimeout(() => {
            guild.members.unban(toBanID).then(() => channel.send(`${toBan} a bien été débanni!`))
        }, delayResolver(delay))
    }
  }
};
