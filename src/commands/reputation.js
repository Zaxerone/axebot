...
module.exports = {
    ...
    async execute(client, { S_User, channel, args, guild, message, user }) {
        ...
        if (!userSearch) return channel.send(`vous avez ${S_User.rep} point de REP`);
        var UserFound = guild.member(message.mentions.users.first() || client.users.find(user => user.username === userSearch) || client.users.get(userSearch));
        if (!UserFound) return channel.send("Je ne trouve pas l'utilisateur dans le serveur !");
        if (UserFound.id === user.id) return channel.send(`Vous ne pouvez pas vous donnez de la réputation !`);
